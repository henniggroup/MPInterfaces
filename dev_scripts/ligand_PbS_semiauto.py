...
strt = Structure.from_file(
    "POSCAR_PbS_bulk_with_vdw")  # using POSCAR of vdW relaxed PbS
mol_struct = Structure.from_file(
    "POSCAR_DMF")  # using POSCAR of vdW relaxed PbS
...
iface = Interface(strt, hkl=hkl, min_thick=min_thick, min_vac=min_vac,
                  ...
                  ligand=DMF, displacement=ads_distance,
                  adatom_on_lig=adatom_on_ligand,
                  adsorb_on_species=slab_species, primitive=True)
...
interface_poscar.write_file(
    "./Interface/POSCAR_PbS100DMF_interface_with_sd.vasp")
