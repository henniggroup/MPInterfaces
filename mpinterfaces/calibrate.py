...
class Calibrate(object):
    ...
    def encut_cnvg(self, encut_list):
        ...
        for encut in encut_list:
            ...
            vis = MPINTVaspInputSet('encut_'+str(encut), self.incar, self.poscar, self.potcar, self.kpoints)
            #the job command can be overrridden in the run method
            job = MPINTVaspJob(["pwd"], final = True, setup_dir=self.setup_dir, job_dir=self.job_dir, vis=vis, auto_npar=False, auto_gamma=False)
...
class CalibrateMolecule(Calibrate):
    ...
    def kpoints_cnvg(self, Grid_type = 'M', kpoints_tuple = ((7, 7, 7), (10, 10, 10)), conv_step = 1):
        ...
        vis = MPINTVaspInputSet('kpoint_'+str(K[0])+'x'+str(K[1])+'x'+str(K[2]), self.incar, self.poscar, self.potcar, self.kpoints)
        job = MPINTVaspJob(["pwd"], final = True, setup_dir=self.setup_dir, job_dir=self.job_dir, vis=vis, auto_npar=False, auto_gamma=False)
        self.jobs.append(job)
